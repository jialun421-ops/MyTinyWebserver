...
- **静态文件服务**：基于 `sendfile` 实现零拷贝响应，减少一次用户态内存拷贝
- **HTTP/1.1 长连接**：支持 keep-alive，单 TCP 连接处理多个请求
- **TCP_NODELAY**：禁用 Nagle 算法，避免与 Delayed ACK 形成 40ms 死锁
...
**环境**：VMware 虚拟机 + Ubuntu 20.04，单核 / 2GB 内存
...
| 测试 | QPS | 平均延迟 | 90% 延迟 |
|---|---|---|---|
| 短连接 c=100 | 11225 | 8.9 ms | 14 ms |
| 长连接 c=100 | 57737 | 1.73 ms | 2 ms |
| 短连接 c=500 | 5186 | 96.4 ms | 140 ms |
